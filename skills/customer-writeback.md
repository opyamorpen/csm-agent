...
4. ONES 新建工作项前调用 `get_issue_fields` 获取目标类型的字段契约，草稿只携带最小必填参数（所属项目、工作项类型、标题、客户信息、描述）。`fieldValues` 必须包含 `JrvswW8P` 及当前客户 option ID；客户 option 按「客户名称」（`field_n1qN0__c__r`，全称）或「售后客户名称」（`field_83f4l__c`，简称）精确唯一解析，解析失败时刷新客户同步，不做模糊归属。
