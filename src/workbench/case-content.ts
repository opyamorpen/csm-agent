...
const docs = (page: string) => `https://docs.ones.cn/wiki/#/team/6mRWUuNv/space/FhmJNPFc/page/${page}`;
// Editorial methods grounded in product documentation, never customer delivery evidence.
const PRACTICES: CasePractice[] = [
  { id: 'intake-governance', title: '统一入口与准入治理', text: '面向多部门共同提报的场景，可先明确需求对象、最小信息集与归口责任，再用统一表单、工作项字段和状态流转承接提报、分派与反馈。复制这一做法的前提是业务部门对准入条件和处理责任达成一致；表单集中并不自动意味着决策集中，还需明确例外事项由谁处理、处理结论如何回到提出方。', sources: [docs('4YQE1RKg'), docs('BHtMALaP')] },
  { id: 'phased-adoption', title: '按业务闭环分期采用', text: '面向上线窗口紧、参与角色多的场景，可优先选择从输入到反馈能够闭合的业务链路开展试点，约定每个角色的操作责任与阶段验收标准，再逐步扩展业务范围。分期边界宜依据业务依赖确定，不能仅按功能菜单切分；进入下一阶段前，需要验证前一阶段的数据是否完整、责任是否落实、用户是否持续采用。', sources: [docs('8TWi7245'), docs('JgBHvnJH'), docs('QLqbR5gz')] },
  { id: 'identity-lifecycle', title: '身份与组织生命周期协同', text: '面向已有统一身份体系的组织，可分别梳理登录验证、用户目录、组织关系与许可分配，再确定各自的权威数据源和同步责任。单点登录解决身份验证，目录同步解决账号与组织数据维护，两者需要配合设计。复制时应核对部署版本、字段匹配、同步范围及离职账号处理规则，并验证账号停用后历史记录的可追溯性。', sources: [docs('DdV5mwEN'), docs('QTMwDKsp'), docs('YS9kSvv9')] },
  { id: 'migration-continuity', title: '存量数据与业务连续性', text: '面向历史台账迁移的场景，可先定义新旧业务对象、状态与人员的映射规则，区分继续推进、归档留存和需要人工核对的数据，再分批验证迁移结果。导入能力只是载体，复制时还需约定抽样核对、异常处理和新旧系统切换责任；不能仅凭导入成功就认定历史关联、统计口径与权限均已正确恢复。', sources: [docs('DBKkjCuY'), docs('6A258nft')] },
  { id: 'role-usability', title: '角色任务与业务语言对齐', text: '面向非研发角色共同使用的平台，可从每类人员日常需要完成的任务出发，统一业务术语、精简表单字段、配置角色权限与常用视图。复制时应保留管理必需的数据和控制点，通过代表性用户的实际任务检验入口、填写和反馈是否顺畅；界面调整的价值需要结合采用行为验证，不能直接等同于学习成本已经下降。', sources: [docs('UcUGNQQd'), docs('Hg9FK1Uq')] },
  { id: 'deployment-boundaries', title: '部署约束与验收责任', text: '面向有明确安全和技术路线要求的组织，可将部署环境、身份接入、数据访问、运维责任和验收依据纳入同一方案边界。复制时需按实际产品版本核对支持范围，把兼容性验证和业务验收分别落实到责任方；选用私有部署本身不能证明已经满足特定监管条款，也不能替代对数据库、接口或安全要求的逐项验证。', sources: [docs('9Yf39csU'), docs('JoUpMJvU')] },
  { id: 'operating-feedback', title: '运营反馈与价值验证', text: '面向需要持续推广的平台，可围绕业务目标确定数据对象、度量口径、观察周期与复核责任，再利用视图、报表和仪表盘支持日常检查。复制时应先建立可比较的基线，区分平台覆盖、实际采用、流程变化与业务结果，结合用户反馈解释变化原因；服务投入、上线数量或功能可用只能说明投入与交付，不能单独证明效率提升或投资回报。', sources: [docs('VjHHDmZF'), docs('HRe1mjzr'), docs('Mc8rpAcG')] },
];
