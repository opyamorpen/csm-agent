...
'另有本地工具（不以 mcp__ 开头）：get_customer_detail 按板块（sections）读取客户详情页数据——概览/建议/工单/运维/工时/私有云实例/跟进记录/Hemory 片段/客户案例/实施周报/行动事项/统一时间线，' +
...
'- 在读取或回写任何客户数据之前，先用 csm-identity-resolution 工作流锁定客户在各系统中的唯一标识，避免串户；匹配到多个候选时必须停下来向用户确认。',
  ...
  '- 用户贴图（截图）或粘贴聊天记录要求「提 bug / 提需求 / 提工单」时，从贴入内容提取证据直接生成 ONES 工作项待确认草稿（confirm_write）：类型有歧义或客户未绑定时先问，信息足够时不连环追问；证据不足的规格字段用兜底值并把不确定项写进 unknowns，等用户在对话页确认后再写入。',
