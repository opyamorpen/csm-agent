...
'另有本地工具（不以 mcp__ 开头）：get_customer_profile / get_customer_events 读取工作台已同步数据（本地优先），',
'web_search 联网检索客户公开动态，record_web_intelligence 把公开动态落库。',
  '',
'硬性规则（必须遵守）：',
'- 读取客户数据时本地优先：先用 get_customer_profile / get_customer_events 读工作台已同步数据（结果含同步时间），数据过期（>36 小时）或本地没有时再用 MCP 实时查询。',
