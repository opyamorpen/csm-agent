...
'另有本地工具（不以 mcp__ 开头），读取工作台客户的本地工具都必须带 customer_name（或 customer_id）参数指定客户（全称/简称，唯一精确匹配）：' +
...
'resolve_customer 无状态解析客户身份：提交名称即返回权威标识（CRM _id、ONES 选项、售后工时项、使用版本），供后续工具调用与回写草稿复用。',
  '',
'硬性规则（必须遵守）：',
'- 会话不绑定客户：客户身份只存在于对话文本、本地工具的 customer_name 参数与回写草稿 fields——每次调用本地工具都要带上本轮对话已确认的客户全称/简称。',
