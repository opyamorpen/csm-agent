...
export const customerDetailTool: Tool = {
  ...
  description:
    '按需读取指定客户的客户详情页数据（本地已同步数据，结果含同步时间）。customer_name/customer_id 指定客户（全称/简称/别名，唯一匹配：精确或唯一子串）。' +
...
export async function makeCustomerDetailResult(
  ...
  if (!customer) {
    return {
      text: `${CUSTOMER_DETAIL_TOOL_NAME}: 未提供或未能唯一解析客户——请带 customer_name（客户全称/简称/别名，唯一匹配；身份未锁定时先完成 csm-identity-resolution）。`,
      isError: true,
    };
  }
