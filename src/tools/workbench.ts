...
export const customerProfileTool: Tool = {
  ...
  description:
    '读取指定客户在工作台已同步的档案聚合（customer_name/customer_id 指定客户，全称/简称/别名唯一匹配：精确或唯一子串）：客户基本信息（名称/行业/健康度/续约日期/合同价值/CSM）、' +
  ...
  parameters: Type.Object({
    customer_name: Type.Optional(Type.String({ description: '客户全称/简称/别名（与工作台客户唯一匹配：精确或唯一子串）' })),
...
export const customerEventsTool: Tool = {
  ...
  description:
    '读取指定客户在工作台已同步的事件时间线（customer_name/customer_id 指定客户，全称/简称/别名唯一匹配：精确或唯一子串；按发生时间倒序）：CRM 跟进记录、ONES 建议/工单/运维工单/工时/私有云实例、' +
...
export interface WorkbenchToolDeps {
  /** 按调用参数解析客户（全称/简称/别名唯一匹配：精确或唯一子串；或按权威 CRM _id 直取）；未唯一命中返回 null。 */
...
function requireCustomer(
  ...
  if (!customer) {
    throw new Error(
      `${toolName}: 未提供或未能唯一解析客户——请带 customer_name（客户全称/简称/别名，唯一匹配；身份未锁定时先完成 csm-identity-resolution）。`,
    );
  }
  return customer;
}
