...
export const customerProfileTool: Tool = {
  ...
  description:
    '读取当前会话所绑定客户在工作台已同步的档案聚合：客户基本信息（名称/行业/健康度/续约日期/合同价值/CSM）、' +
...
export const customerEventsTool: Tool = {
  ...
  description:
    '读取当前会话所绑定客户在工作台已同步的事件时间线（按发生时间倒序）：CRM 跟进记录、ONES 建议/工单/运维工单/工时/私有云实例、' +
...
function requireCustomer(deps: WorkbenchToolDeps, toolName: string): { id: string; name: string } {
  const customer = deps.getCustomer();
  if (!customer) {
    throw new Error(
      `${toolName}: 当前会话未绑定客户。请让用户从客户详情页发起对话，或先提供客户名称完成身份解析。`,
    );
  }
  return customer;
}
...
export function makeWorkbenchToolHandlers(deps: WorkbenchToolDeps): {
  profile: () => Promise<{ text: string; isError?: boolean }>;
  events: (args: Record<string, unknown>) => Promise<{ text: string; isError?: boolean }>;
} {
  return {
    profile: async () => {
      const customer = requireCustomer(deps, CUSTOMER_PROFILE_TOOL_NAME);
    ...
    events: async (args) => {
      const customer = requireCustomer(deps, CUSTOMER_EVENTS_TOOL_NAME);
