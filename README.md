...
仓库开发时可用 `npm run cli -- <命令>`，行为与全局命令相同。服务不在默认端口时设置 `CSM_BASE_URL=http://127.0.0.1:3212`。`api` 是所有 API 的通用诊断入口；核心工作流仍需提供独立、可读的 CLI 命令。后续功能调整必须同步更新 CLI、`capabilities` 清单、帮助、测试和本说明，不能只完成页面操作。

CRM 客户集合以「CSM售后客户」中的“客户名称”为业务来源，CRM 记录 `_id` 仍是工作台唯一客户主键。当前纷享 MCP 将这些记录暴露为 `AccountObj`，同步只接收已分配 CSM/已交接 CSM 的记录；客户名称用于解析 ONES 的「客户信息」选项，重名或非唯一匹配不会自动归属。
...
五类工作项统一使用「客户信息」字段 `JrvswW8P`。同步先用 CRM 客户名称精确解析该字段的 option ID，再通过 ONESQL 查询；标题模糊匹配不再作为自动归属依据。
...
真实交付边界分为：本地测试与构建、三套 MCP 同步、浏览器页面验收、ONES Wiki 实际发布、企业微信桌面端/移动端待办创建与状态回写。后两项必须使用真实权限与凭据验证，不能由本地配置存在代替。
