...
- 风险预警名单：满足任一触发条件即进入待处理预警——①近 30 天无 ONES 工作项新增/更新且无新增工时（须有过历史记录，从未使用 ONES 的客户不预警）；②公开动态检索到该客户负面信息（收入下降/罚款/投诉/负面评价等，回看 90 天与风险 web 维度同口径）。预警与风险重算同源触发（每日同步/手动刷新/公开动态落库后）。CSM 可消除风险并必须填写原因/动作（写审计）；条件自动解除（恢复活动、负面移出窗口）由系统标记；消除后情况无新变化不重报，新沉默周期或新负面证据出现才重新预警。Web 左侧导航「预警」页（待处理/已消除双 tab）与客户详情预警横幅均可消除，CLI 同款：`csm-agent alerts` / `csm-agent alerts resolve <预警ID> --note "..."`。
...
csm-agent alerts [--status active|resolved|all] # 风险预警名单：近 30 天无 ONES 工作项新增/更新且无新增工时、或公开动态检索到该客户负面信息（收入下降/罚款/投诉等）的客户自动进入；默认待处理
