...
- 客户案例：按可直接公开传播的标准生成五段客户叙事（背景→痛点/现状/挑战→需求/要求→解决方案→价值），五段有明确篇幅契约（背景 150~400 字、方案 2~5 个举措自然段、列表条目两层结构），素材不足时宁少勿注水。后台任务基于客户公开档案、全量源事件、已确认 Hemory 片段和 Evidence，结合项目管理/需求管理/知识管理/招投标/中标采购五角度公开检索；服务端预计算交付事实统计（合作起点与时长、建议/工单/运维已完成交付量、工单解决时效、工时投入）作为可引用素材注入——模型可直接引用这些数字作事实陈述，但禁止推导完成率或提升百分比，低完成量只写已完成部分；说话人、否定词、时态与结构化状态共同决定章节，转写匿名分轨（speaker_N）与「我/我们」视角按客户侧归类，未确认信息只进内部 `unknowns`，正文不出现占位词。每条主张以 `claim_evidence` 绑定来源原文（摘录必须能定位到引用来源的连续原文——防编造是阻断项；说话人角色判定降级为非阻断告警，由 CSM 复核），生成上下文固化为 `context_snapshot`。生成后计算素材覆盖度（已交付记录/客户正面反馈被正文实际引用的比例），主要素材池大量闲置时自动追加一次「补充完善」生成（补写失败保留第一稿），覆盖率与补写情况随草稿 `coverage` 字段持久化并在 `case show` 输出。模型调用显式给足输出预算（长 JSON + 推理 token 共享 completion 上限，服务端默认 8k 会截断）并放宽首字节超时（大上下文常见 40~60s）。支持对话精修与五段正文编辑，服务端始终保留内部证据；编辑与精修写回走非阻断护栏（条目数失控、单条超长、内部信息残留时随响应返回提醒）。发布前公开检查会提示空章节、无来源数字、非客户反馈、未完成事项、敏感信息、交付统计披露口径、其他客户名称、过期上下文和授权复核等问题，均为非阻断警告。
